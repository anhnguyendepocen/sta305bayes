...
\noindent \textit{Propriétés utiles :}
\begin{itemize} 
	\item La densité de probabilité de la
loi Beta de paramètres $a>0$ et $b>0$ évaluée en $\theta$ est donnée par%
\[
\text{Beta}(\theta;a,b)=\frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}\theta
^{a-1}(1-\theta)^{b-1}%
\]

	\item Soit $\theta$ une variable aléatoire suivant une loi Beta de paramètres $a$ et
$b$. On a alors $$\mathbb{E}[\theta]=\frac{a}{a+b}$$
\end{itemize}\bigskip
